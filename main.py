...
# Load Datasets
symp_df = pd.read_csv("Datasets/symtoms_df.csv")
...
# Helper Function
def helper(dis):
    descr = descriptions[descriptions['Disease'] == dis]['Description']
...
def predict():
    if request.method == 'POST':
        ...
        descr, prec, med, diet, wrkout = helper(predicted_disease)
        ...
        my_med = []
        for i in med[0].split(","):
           my_med.append(i)

        my_diet = []
        for i in diet[0].split(","):
            my_diet.append(i)

        return render_template("index.html", predicted_disease = predicted_disease, dis_descr = descr, dis_prec = my_prec, dis_med = my_med, dis_wrkout = wrkout, dis_diet = my_diet)
